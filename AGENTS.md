...
- Paid production progresses only through `deepseek_probe -> pilot5 -> pilot10 -> pilot25 -> batch50 -> remainder`. Every live stage performs its own zero-paid local preparation and launch projection before constructing the DeepSeek client. `shadow` remains an optional zero-paid offline diagnostic and is never promotion evidence or a prerequisite for `deepseek_probe`. `deepseek_probe` must use DeepSeek, has a hard RMB 100 stage cap, and must complete fresh before `pilot5`. No local model installation or invocation is required by this release path. Promotion otherwise requires the complete fresh sample for the current stage; recovered, resumed, migrated, or repackaged papers are audit evidence only and never promotion evidence.
- Any new pilot candidate must be selected from the current zero-cost source-prefilter manifest, with a live record showing `publication_allowed: true` and `eligible: true`; manual selection from a TOC/layout lane is insufficient. A low-risk pilot additionally requires `pages <= 10`, `images == 0`, `drawings <= 200`, `numeric_citations <= 10`, `citation_ranges <= 1`, `reference_pages <= 2`, and `contents_pages == 0`. If no untouched candidate satisfies the manifest, do not spend API budget by retrying an unchanged paper; stop at the prefilter report and report that the low-risk pool is exhausted.
- The separate `figure_passthrough_medium` tier may admit at most 2 embedded images and 500 vector objects only when all figure/plot/table interiors remain source-verbatim under the figure-text contract; it is never a permission to translate text inside graphics.
- The separate `text_only_long` tier may admit up to 40 pages and 150 numeric citations only when the source PDF has `images == 0`, `drawings == 0`, and no Contents page; it remains a text-only lane and does not relax any References, cross-page residue, or visual gates.
- The separate `vector_passthrough_long` tier may admit up to 30 pages and 500 vector objects only when `images == 0`, no Contents page is present, and every vector/figure interior remains source-verbatim; it requires a fresh single-paper pilot before any cohort expansion.
- The separate `citation_dense_passthrough_medium` tier may admit up to 20 pages, 2 embedded images, 500 vector objects, 200 numeric citations, 20 citation ranges, and 3 reference pages only when no Contents page is present, every figure/table interior remains source-verbatim, and the References protection/self-check gates remain strict. It requires one fresh single-paper pilot before any cohort expansion; the first candidate must minimize pages, images, drawings, citation ranges, and reference pages within the untouched tier pool.
...
- Every numeric citation marker in model-bound user text must be replaced by a unique transport token before the DeepSeek request. A successful response must return every token exactly once and in the original order and must not invent any unlocked numeric citation marker; the proxy restores the exact source marker bytes only after this check and records lock/validation/failure counts in the finish receipt.
...
- An unchanged quarantined input must not re-enter a paid queue. Clearing quarantine requires a changed input fingerprint or explicit operator review. Paid expansion beyond one paper remains frozen until the live DeepSeek probe passes all local preflight, semantic, structural, and visual gates.
